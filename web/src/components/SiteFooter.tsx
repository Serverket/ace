...
interface SiteFooterProps {
  onLegal: (type: 'privacy' | 'tos') => void;
  // In app mode the anchors switch bottom-nav tabs instead of scrolling
  onQuotesClick?: () => void;
  onCalcClick?: () => void;
}

export const SiteFooter: React.FC<SiteFooterProps> = ({ onLegal, onQuotesClick, onCalcClick }) => {
  return (
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 pb-10 border-b-4 border-black">
          ...
          <div className="flex flex-col items-end justify-center gap-4 text-xl font-black text-black">
            <a
              href="#cotizaciones"
              onClick={onQuotesClick ? (e) => { e.preventDefault(); onQuotesClick(); } : undefined}
            ...
            <a
              href="#calculadora"
              onClick={onCalcClick ? (e) => { e.preventDefault(); onCalcClick(); } : undefined}
              className="hover:bg-black hover:text-white px-2 py-1 transition-colors border-2 border-transparent hover:border-black"
            >
              CALCULADORA
            </a>
        ...
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm font-bold text-black text-center sm:text-left">
          <p className="flex flex-wrap items-center justify-center sm:justify-start gap-2 bg-white px-4 py-2 border-2 border-black shadow-brutal-sm">
            ...
            <span>/ GPLV3</span>
            <button onClick={() => onLegal('tos')} className="hover:bg-black hover:text-white px-1 transition-colors border-b-2 border-black">TÉRMINOS</button>
            <button onClick={() => onLegal('privacy')} className="hover:bg-black hover:text-white px-1 transition-colors border-b-2 border-black">PRIVACIDAD</button>
          </p>
